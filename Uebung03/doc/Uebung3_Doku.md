...
Deswegen wurde die Architektur um die fehlenden Werte von STD_ulogic erweitert. Hier wurden die schwache '1' also 'H' gleich wie die '1' implementiert. Gleiches wurde mit dem Pegel 'L' für die schwache '0' vollzogen. 
...
Werden diese Pegel nicht berücksichtigt wird immer der Wert der im when others-> steht bzw. im else Zweig der if steht.
...
Im gegensatz dazu erzeugt die Beschreibung mittels if zwei Multiplexer entstehen. Dies ist notwendig, da beim if-elsif-else die Reihenfolge der Abfrage für das Ergebnis entscheidend ist, dies wird mittels der 2 Multiplexer dargestellt.
...
In der Synthese sind zusätzlich zu den Warnings weil kein clock definiert wurde noch weitere Warnings zu den Schwachen Pegel 'H', 'L' bzw. 'U'. Hier wird die Warning ausgegeben, dass diese Pegel in der Synthese ignoriert werden. Dies ist auch logisch, da nur Pegel '1', '0' und 'Z' synthetisierbar sind und somit alle anderen für die Synthese irrelevant sind.
...
Dies hängt wahrscheinlich damit zusammen, dass manche Übergänge im Design nicht vorkommen. Das heist im konkreten Design, dass kein Übergang von Rising Edge am Key[0] zu einer Rising Edge an der LEDR[0] bewirkt.

Der Name für den Datasheet Report stammt wahrscheinlich daher, dass dieser ein art Datenblatt für das synthetisierte Design erstellt.

Durch verstellen der Operation Conditions ändern sich die Durchlaufzeiten nicht.
...
Das Propagation-Delay wurde bei der längsten Zeit um 11.3% länger geworden, bei der kürzesten Zeit um 11.1%.
